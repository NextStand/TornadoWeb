...
from Wechat.lib.WechatRequest import WechatRequest
from Wechat.Exceptions import NeedParamError, ParamTimeout
from Wechat.lib.crypto import BasicCrypto
...
class WechatConf(object):
    ...
    def __init__(self, **kwargs):
        ...
        :param kwargs: 配置信息字典, 可用字典 key 值及对应解释如下: 
            ...
            'encoding_aes_key': 对称加密算法EncodingAESKey 值 (传入此值必须保证同时传入 token, appid, 否则抛出异常)
            'access_token_getfunc': access token 获取函数 (用于单机及分布式环境下)
            'access_token_setfunc': access token 写入函数 (用于单机及分布式环境下)
            'access_token_refreshfunc': access token 刷新函数 (用于单机及分布式环境下)
            ...
            'jsapi_ticket_getfunc': jsapi ticket 获取函数 (用于单机及分布式环境下)
            'jsapi_ticket_setfunc': jsapi ticket 写入函数 (用于单机及分布式环境下)
            'jsapi_ticket_refreshfunc': jsapi ticket 刷新函数 (用于单机及分布式环境下)
        ...
        self.__access_token_refreshfunc = kwargs.get(
        ...
        self.__jsapi_ticket_refreshfunc = kwargs.get(
    ...
    def token(self, token):
        """ 设置当前 Token """
        self.__token = token
        # 因为加密算法中需要token,改动 Token 需要重新更新 Crypto
        self._update_crypto()
    ...
    def jsapi_ticket(self):
        """ 获取当前 jsapi ticket 值, 本方法不会自行维护 jsapi ticket 有效性 """
        ...
        if self.__jsapi_ticket:
            ...
            if self.__jsapi_ticket_expires_at - now > 60:
                ...
                raise ParamTimeout('jsapi ticket timeout')
    ...
    def _check_access_token(self):
        ...
        :raises NeedParamError: access_token 参数没有在初始化的时候提供
        """
        if not self.__access_token:
            raise NeedParamError(
                'Please provide access_token parameter in the construction of class.')
    ...
    def _update_crypto(self):
        ...
        if self.__encrypt_mode in ['compatible', 'safe'] and self.__encoding_aes_key is not None:
            if self.__token is None or self.__appid is None:
                raise NeedParamError(
                    'Please provide token and appid parameters in the construction of class.')
            self.__crypto = BasicCrypto(
                self.__token, self.__encoding_aes_key, self.__appid)
        else:
            self.__crypto = None
