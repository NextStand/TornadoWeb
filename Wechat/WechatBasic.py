...
from .core.WechatConf import WechatConf
from .lib.WechatRequest import WechatRequest
from .lib.Parse import XMLStore
from .WechatMsg import MESSAGE_TYPES, UnknownMessage
from Exceptions import ParseError, NeedParseError, OfficialAPIError, NeedParamError
...
class WechatBasic(WechatBase):
    ...
    def __init__(self, token=None, appid=None, appsecret=None, partnerid=None,
        ...
        if conf is not None:
            ...
            # 针对将 WechatConf 实例作为第一个参数传入的容错处理
            self.__conf = token
        else:
    ...
    def parse_data(self, data, msg_signature=None, timestamp=None, nonce=None):
        ...
        if self.conf.encrypt_mode == 'safe':
            if not (msg_signature and timestamp and nonce):
                raise ParseError(
                    'must provide msg_signature/timestamp/nonce in safe encrypt mode')
    ...
    def get_jsapi_ticket(self):
        ...
        **仅为兼容 v0.6.0 以前版本使用, 自行维护 jsapi_ticket 请使用 jsapi_ticket_setfunc 和 jsapi_ticket_getfunc 进行操作**
        ...
        """
        return self.request.get('https://api.weixin.qq.com/cgi-bin/ticket/getticket',data=dict(type='jsapi'))
    ...
    def getTokenByCode(self, code):
        """
        OAuth2.0鉴权时获取access_token和openid，构造WechatBasic实例的时候要传递appid和appsecret
        详情请参考 https://mp.weixin.qq.com/wiki?t=resource/res_main&id=mp1421140842
        :param code: 前端授权时候的code
        :return: 携带JSON数据包的异步Future
        """
        if all((self.__conf.appid, self.__conf.appsecret)):
            return self.request.get(
                url='https://api.weixin.qq.com/sns/oauth2/access_token',
                data={
                    'appid': self.__conf.appid,
                    'secret': self.__conf.appsecret,
                    'code': code,
                    'grant_type': 'authorization_code'
                }
            )
        else:
            raise NeedParamError(message='构造参数中缺少appid或appsecret')

    def getUserInfoByOAuth(self, access_token, openid, lang='zh_CN'):
        """
        获取用户基本信息，OAuth2.0鉴权方式，前提是前端生成code时候的scope=snsapi_userinfo，该方法才有效
        详情请参考 https://mp.weixin.qq.com/wiki?t=resource/res_main&id=mp1421140842
        :param access_token: 授权token，非基础支持中的token,构造WechatBasic实例的时候不要传递access_token参数
        :param openid: 用户的id
        :param lang: 返回国家地区语言版本，zh_CN 简体，zh_TW 繁体，en 英语
        :return: 携带JSON数据包的异步Future
        """
    ...
    def send_article_message(self, user_id, articles=None, media_id=None):
        ...
        if articles is None and media_id is None:
            raise TypeError(
                'must provide one parameter in "articles" and "media_id"')
    ...
    def send_template_message(self, user_id, template_id, data, url='', topcolor='#FF0000'):
        """
        发送模板消息
        详情请参考 https://mp.weixin.qq.com/wiki?t=resource/res_main&id=mp1433751277
