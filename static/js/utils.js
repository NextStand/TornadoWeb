...
(function (window, document, $) {
     ...
     * 浏览器类型判断构造函数
     */
    window.qcdn = 'http://wx2.chengyujiao.com/';    //CDN加速服务器
    ...
    window.barray = {
        ...
        window.bdate = {
             ...
             * 调用方法：var str = bDate.fmtdate(new Data(),"y-M-d")
             * @param {Date} data
            ...
            fmtdate: function (data, pattern) {
                ...
                if (pattern.length >= 5) {
                    ...
                    if (barray.arrContains(pattern, "M", false) || tmArray.arrContains(pattern, "年", false)) {
                    ...
                    if (barray.arrContains(pattern, "H", false) || barray.arrContains(pattern, "分", false)) {
        ...
        window.http = {
            ...
            _parse: function (params) {
                ...
                var _authtoken = hex_md5(
                    (bwin.getCookie("_BAUTH") ? bwin.getCookie("_BAUTH") : "000000") +
                    (bwin.getCookie("_BUUID") ? bwin.getCookie("_BUUID") : "000000") +
                    (_time) + (this._orderParams(obj))).toUpperCase();
                obj._timestamp = _time;
                obj._authtoken = _authtoken;
                for (var key in obj) {
                    if (key != '_timestamp' && key != '_authtoken') {
                        obj[key] = bstr.tohex(obj[key])
                    }
                }
                return obj;
            },
            ...
            get: function (api, params, callback1, callback2) {
                $.ajax({
                    ...
                    success: function (data) {
                        callback1 && callback1 instanceof Function && callback1(data)
                    },
                    error: function (data) {
                        if(data.status == 403){
                            alert('无权访问或登录过期，请确认身份后重新登录')
                        }
                        callback2 && callback2 instanceof Function && callback2(data)
                    }
                })
            },
            /**
            ...
            post: function (api, params, callback1, callback2) {
                ...
                $.ajax({
                    ...
                    beforeSend: function (XMLHttpRequest) {
                        XMLHttpRequest.setRequestHeader("X-XSRFToken", bwin.getCookie("_xsrf"));
                    },
                    success: function (data) {
                        callback1 && callback1 instanceof Function && callback1(data)
                    },
            ...
            delete: function (api, params, callback1, callback2) {
                ...
                $.ajax({
                    ...
                    error: function (data) {
                        if(data.status == 403 && api != '/api/login'){
                            alert('无权访问或登录过期，请确认身份后重新登录')
                        }
                        callback2 && callback2 instanceof Function && callback2(data)
                    }
            ...
            /**
             * 图片上传云服务
             * @param {Blob} file 上传文件二进制对象
             * @param {Function} complete 上传完成的回调，回到参数为一个包含key和hash的对象默认这两个值是一样的
                            ，加上window下的qcdn属性可获取目标对象
             * @param {Function} error 上传失败的回调，回调参数为错误对象
             * @param {Function} next 上传进度改变的回调，回调参数为一个包含总大小、已上传大小，进度百分比的对象
             * @param {Function} callback2 token获取失败的回调
             */
            uploadimg: function (file, complete, error, next, callback2) {
                if (file.type === "image/jpeg" || file.type === "image/png") {
                    this.get('/upload/token', null, data => {
                        if (data.errcode == 0) {
                            var observable = qiniu.upload(file, null, data.data)
            ...
            $(this).find('input,select').each(function (index, item) {
                ...
                if (inputName != undefined) {
                    ...
                    if ($(item).attr('date-format') != undefined) {
                        ...
                        if (!isNaN(val)) { //number
                            val = bDate.getFormatDateByLong(val, $(item).attr('date-format'));
                        } else if (val instanceof Date) {
                            val = bDate.getFormatDate(val, $(item).attr('date-format'));
                        }
                    }
                    $(item).val(val);
